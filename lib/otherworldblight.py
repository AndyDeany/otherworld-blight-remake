...
try:
    ...
    try:
        import pygame._view
    except: # sometimes this isn't necessary
        pass
...
while ongoing:
    ...
    # Resetting inputs
    try:
        # Resetting the mouse inputs to 0 (off) so the computer doesn't think the mouse is still pressed
        left = 0
        middle = 0
        right = 0
    ...
    try:
        for event in pygame.event.get():                    # i.e. Whenever the user does something
            ...
            elif event.type == pygame.MOUSEBUTTONUP:        # Checking whether a mouse button was released
                mouse_state = pygame.mouse.get_pressed()
                if left_held and not mouse_state[0]:
                    left = 1
                    left_held = 0  # Showing that the left mouse button has been released
